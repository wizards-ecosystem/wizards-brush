...
`backend/app/finishing.py` is a registry of named processors behind an optional
`finish_steps` param that inline finishing runs after the unchanged `finish`
preset. Built-ins are exact resize, alpha-preserving upscale and face restore,
...
| `POST /api/variant-sets`, `GET /api/variant-sets`, `GET/DELETE /api/variant-sets/{id}` | Create (idempotent on `request_id`), list, detail, delete |
| `GET /api/variant-sets/{id}/items[/{item_id}]` | Items with live job state |
...
so a similarity check over the stored CLIP embeddings can be added later. A
future segmentation tool creates masks by saving an image asset with the
`mask` generator, which recipes already reference by id. New third-party reuse must be checked against
